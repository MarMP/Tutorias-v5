...
public class Profesores implements IProfesores {
	...
	public void insertar(Profesor profesor) throws OperationNotSupportedException {
		if (profesor == null) {
			throw new IllegalArgumentException("ERROR: no se puede insertar un profesor nuulo.");
		}
		if (buscar(profesor) != null) {
			throw new OperationNotSupportedException("ERROR: Ya existe un profesor con el mismo DNI.");
		} else {
	...
	public Profesor buscar(Profesor profesor) {
		...
		Document documentoProfesor = coleccionProfesores.find().filter(eq(MongoDB.DNI, profesor.getDni())).first();
