...
public class MongoDB {
	...
	public static Profesor getProfesor(Document documentoProfesor) {
		...
		return new Profesor(documentoProfesor.getString(NOMBRE), documentoProfesor.getString(DNI),
				documentoProfesor.getString(CORREO));
	}
	...
	public static Alumno getAlumno(Document documentoAlumno) {
		...
		return new Alumno(documentoAlumno.getString(NOMBRE), documentoAlumno.getString(CORREO),
				Integer.valueOf(documentoAlumno.getString(EXPEDIENTE).split("_")[2]));
	}
	...
	public static Document getDocumento(Sesion sesion) {
		...
		return new Document().append(TUTORIA, getDocumento(tutoria)).append(FECHA, FORMATO_DIA.format(fecha))
				.append(HORA_INICIO, FORMATO_HORA.format(horaInicio)).append(HORA_FIN, FORMATO_HORA.format(horaFin))
	...
	public static Document getDocumento(Cita cita) {
		...
		return new Document().append(SESION, getDocumento(sesion)).append(ALUMNO, getDocumento(alumno)).append(HORA,
				FORMATO_HORA.format(cita.getHora()));
	}

	public static Document getCriterioOrdenacionCita() {
		return new Document().append(SESION_TUTORIA_PROFESOR_DNI, 1).append(SESION_FECHA, 1).append(HORA, 1);
	}
