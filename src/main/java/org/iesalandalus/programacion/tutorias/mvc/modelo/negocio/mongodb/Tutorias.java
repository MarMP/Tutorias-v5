...
public class Tutorias implements ITutorias {
	...
	public List<Tutoria> get(Profesor profesor) {
		List<Tutoria> tutorias = new ArrayList<>();
		for (Document documentoTutoria : coleccionTutorias.find().filter(eq(MongoDB.PROFESOR_DNI, profesor.getDni()))
				.sort(MongoDB.getCriterioOrdenacionTutoria())) {
			tutorias.add(MongoDB.getTutoria(documentoTutoria));
		}
		return tutorias;
	}
	...
	public Tutoria buscar(Tutoria tutoria) {
		...
		Document documentoTutoria = coleccionTutorias.find().filter(
				and(eq(MongoDB.NOMBRE, tutoria.getNombre()), eq(MongoDB.PROFESOR_DNI, tutoria.getProfesor().getDni())))
				.first();
	...
	public void borrar(Tutoria tutoria) throws OperationNotSupportedException {
		...
		if (buscar(tutoria) != null) {
			coleccionTutorias.deleteOne(and(eq(MongoDB.NOMBRE, tutoria.getNombre()),
					eq(MongoDB.PROFESOR_DNI, tutoria.getProfesor().getDni())));
		} else {
